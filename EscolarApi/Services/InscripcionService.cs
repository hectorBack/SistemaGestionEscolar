    ...
    public class InscripcionService : IInscripcionService
    {
        ...
        public async Task<InscripcionResponse> Inscribir(InscripcionRequest request)
        {
            ...
            try
            {
                ...
                var horariosAlumno = await _context.Inscripciones
                    ...
                    .Select(i => new
                    {
                        ...
                        .ToListAsync(); // Traemos a memoria para asegurar la comparación de TimeSpan

                // 2. Validamos el choque en memoria (C# maneja TimeSpan sin problemas)
                var tieneChoque = horariosAlumno.Any(h =>
                    h.DiaSemana == cursoNuevo.DiaSemana &&
                    cursoNuevo.HoraInicio < h.HoraFin &&
                    cursoNuevo.HoraFin > h.HoraInicio);
